    ...
    <div className="relative">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="mt-2 font-headline text-[clamp(2rem,5vw,3rem)] tracking-[-0.04em] text-[var(--foreground)]">Knowledge Graph</h2>
        </div>
      ...
      {selectedNode ? (
        <aside className="fixed inset-x-3 bottom-[calc(env(safe-area-inset-bottom,0px)+5.5rem)] z-50 rounded-[24px] border border-[color:var(--panel-border)] bg-[color:var(--glass-surface-strong)] p-5 shadow-[var(--glass-shadow-deep)] backdrop-blur-2xl md:inset-x-auto md:bottom-auto md:right-4 md:top-24 md:w-full md:max-w-[320px]">
          ...
          <div className="mt-5 flex flex-col gap-2">
            <Link
              href={`${getHubHref("study", "notes")}&topic=${encodeURIComponent(selectedNode.data.label)}&subject=${encodeURIComponent(selectedNode.data.subject)}`}
              className={cn(buttonVariants({ variant: "default" }))}
            >
              Generate Note
            </Link>
            <Link
              href={`${getHubHref("test", "quiz")}&topic=${encodeURIComponent(selectedNode.data.label)}&subject=${encodeURIComponent(selectedNode.data.subject)}`}
              className={cn(buttonVariants({ variant: "outline" }))}
            >
              Quiz Me
            </Link>
            <Link
              href={`${getHubHref("test", "flashcards")}&topic=${encodeURIComponent(selectedNode.data.label)}`}
              className={cn(buttonVariants({ variant: "outline" }))}
            >
              Flashcards
            </Link>
