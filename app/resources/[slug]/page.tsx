...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  const entry = await getContentEntry("resources", params.slug);
  if (!entry) {
    return {};
  }
  ...
  return {
    ...
    openGraph: {
      title: `${entry.title} — StudyOS Resources`,
      description: entry.description,
      url: entry.path,
      type: "article",
      ...
      <main className="min-h-screen pb-12">
        ...
        <article className="mx-auto max-w-4xl px-6 pb-16 pt-4">
          <div className="glass-card p-6 md:p-8">
            <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-[var(--tertiary-foreground)]">Resource</p>
