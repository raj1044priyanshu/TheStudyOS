  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass-card flex flex-wrap gap-2 p-5">
        <Input
          data-tour-id="mindmap-topic-input"
          value={topic}
          onChange={(event) => setTopic(event.target.value)}
          placeholder="Topic"
          className="max-w-md"
        />
        <Button data-tour-id="mindmap-example-fill" type="button" variant="ghost" onClick={useExample}>
          Use Example
        </Button>
        <Button data-tour-id="mindmap-generate" onClick={generate} disabled={!topic.trim() || loading}>
          {loading ? "Generating..." : "Generate"}
        </Button>
        <Button data-tour-id="mindmap-export-png" variant="outline" onClick={exportPng}>
          Export PNG
        </Button>
      </div>
